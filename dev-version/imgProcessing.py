...
'''
def prepareIMG(UPLOAD_FOLDER, OUTPUT_FOLDER, filename):
...
def createBibstruct(filename):    
    ...
    if htmlbool:
        ...
        for ip,tp in enumerate(xmltags):
            ...
            #line = str(tp).encode("utf8").replace("&apos;", "'").replace("&amp;", "&").replace("&quot;", '"')
            line = str(tp).decode("utf8").replace("&amp;lt;", "<").replace("&amp;gt;", ">").replace("&amp;apos;", "'").replace("&apos;", "'").replace("&amp;", "&").replace("&quot;", '"').replace("&lt;", "<").replace("&gt;", ">").replace("&#63;", '?')
            taghtmlsoup = htmlsoup.find_all('span', attrs={'class':'ocr_line'})
            ...
            for i, t in enumerate(taghtmlsoup):
                if i <= processed_line_number:
                    ...
                    continue
                if t.text.replace("\\", "")[:-1] in line and len(t.text) >= 10:
                    # print ""
                    # print ""
                    # print "++++++++++++++++++++++++++++++++++++++++++++++"
                    # print "line: ", line
                    # print "line no.",i+1,": ",t.text.replace("\\", "")
                    # print "++++++++++++++++++++++++++++++++++++++++++++++"
                    starttag = ''
                    endtag = ''
                    try:
                        if ((i + 1) < len(taghtmlsoup)) and (taghtmlsoup[i + 1].text.replace("\\", "")[:-1] in line):
                            starttag = taghtmlsoup[i]['title']
                            endtag = taghtmlsoup[i + 1]['title']
                            unmatched_lines = 0
                            ...
                            try:
                                while unmatched_lines < 2:
                                    ...
                                    if ((i + matching_lines_iterator) < len(taghtmlsoup)) and (
                                                taghtmlsoup[i + matching_lines_iterator].text.replace("\\", "")[:-1] in line):
                                        starttag = taghtmlsoup[i]['title']
                                        ...
                                        # print "Here 3"
                                        #print "Multiple Else: ",taghtmlsoup[i + matching_lines_iterator].text.replace("\\", "")
                                        unmatched_lines += 1
                ...
                """else:
                    #print "In else, line: ", line
                    print "In else, t.text line no.", i + 1, ": ", t.text.replace("\\", "")
                    #if t.text[:-1] in line:
                    #    #print "In else: Found..."
                    #else:
